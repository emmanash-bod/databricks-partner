    ...
    <>
      ...
      <section className="relative gradient-hero overflow-hidden">
        ...
        <div className="relative container-max section-padding pt-24 sm:pt-32 lg:pt-40 pb-16">
          <p className="text-sm font-semibold text-orange uppercase tracking-wider mb-4">Relationship Building</p>
        ...
        <div className="container-max">
          <div className="space-y-12">
            {venues.map((venue, i) => (
              <div
                key={venue.name}
                className={`grid grid-cols-1 lg:grid-cols-2 gap-8 items-start p-8 rounded-2xl border border-navy/5 hover:shadow-lg transition-shadow duration-300 ${
                  i % 2 !== 0 ? "lg:direction-rtl" : ""
                }`}
              >
                {/* Image placeholder */}
                <div className={`relative aspect-[4/3] rounded-xl overflow-hidden bg-gradient-to-br from-navy-400/80 to-navy ${i % 2 !== 0 ? "lg:order-2" : ""}`}>
                ...
                <div className={i % 2 !== 0 ? "lg:order-1" : ""}>
                  <div className="flex items-center gap-2 mb-3">
                    <span className="px-3 py-1 text-xs font-medium text-orange bg-orange/10 rounded-full">{venue.type}</span>
                    <span className="text-xs text-navy-200">{venue.capacity}</span>
                  </div>
                  <h3 className="text-2xl font-bold text-navy mb-3">{venue.name}</h3>
                  <p className="text-navy-200 leading-relaxed mb-6">{venue.description}</p>

                  {/* Sample Agenda */}
                  <div className="mb-6">
                    <h4 className="text-sm font-bold text-navy uppercase tracking-wider mb-3">Sample Agenda</h4>
                    <div className="space-y-2">
                      {venue.agenda.map((a) => (
                        <div key={a.time} className="flex gap-4 items-center">
                          <span className="text-xs font-mono text-orange w-16 shrink-0">{a.time}</span>
                          <div className="w-2 h-2 rounded-full bg-orange/30 shrink-0" />
                          <span className="text-sm text-navy-200">{a.item}</span>
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      <section className="bg-gray-50 section-padding">
        <div className="container-max">
          <div className="text-center mb-12">
            <p className="text-sm font-semibold text-orange uppercase tracking-wider mb-3">Calendar</p>
            <h2 className="text-3xl sm:text-4xl font-bold text-navy mb-4">Upcoming Events</h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {upcomingEvents.map((evt) => (
              <div key={evt.title} className="p-6 rounded-2xl bg-white border border-navy/5 hover:shadow-lg transition-shadow duration-300">
                <div className="text-sm font-mono text-orange mb-3">{evt.date}</div>
                <h3 className="text-lg font-bold text-navy mb-2">{evt.title}</h3>
      ...
      {/* ── CO-SPONSOR FORM ──────────────────────────────────────── */}
      <section id="cosponsor" className="gradient-navy section-padding">
        <div className="container-max">
          <div className="max-w-2xl mx-auto text-center">
            <p className="text-sm font-semibold text-orange uppercase tracking-wider mb-3">Get Involved</p>
            ...
            <form className="space-y-4 text-left" action="#">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    className="w-full px-4 py-3 rounded-lg bg-white/5 border border-white/10 text-white placeholder:text-white/30 focus:border-orange/50 focus:outline-none transition-colors"
              ...
              <div>
                <label className="block text-sm text-white/60 mb-1.5">Preferred Venue</label>
                <select className="w-full px-4 py-3 rounded-lg bg-white/5 border border-white/10 text-white focus:border-orange/50 focus:outline-none transition-colors">
              ...
              <div>
                <label className="block text-sm text-white/60 mb-1.5">Details / Notes</label>
                <textarea
                  rows={3}
                  className="w-full px-4 py-3 rounded-lg bg-white/5 border border-white/10 text-white placeholder:text-white/30 focus:border-orange/50 focus:outline-none transition-colors"
